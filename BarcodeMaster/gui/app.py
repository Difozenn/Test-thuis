...
from gui.panels.archief_panel import ArchiefPanel
...
class MainApp(tk.Frame):
    def __init__(self, parent, service_status):
        ...
        icon_archief = load_image_safe(get_resource_path('assets/archief.png'), fallback_text="ARCH")

        self.tab_icons = [icon_scanner, icon_db, icon_help, icon_admin, icon_settings, icon_archief]
        self.panels = [
            ScannerPanel,
            DatabasePanel,
            HelpPanel,
        ...
        self.panel_names = ["Scanner", "Database", "Help", "Admin", "Settings", "Archief"]
    ...
    def switch_panel(self, idx):
        ...
        # Notify current panel it's being deactivated
        if self.active_panel is not None:
            if hasattr(self.active_panel, 'on_panel_deactivated'):
                try:
                    self.active_panel.on_panel_deactivated()
                except Exception as e:
                    print(f"Error in on_panel_deactivated: {e}")
        ...
        # Notify new panel it's being activated
        if hasattr(panel, 'on_panel_activated'):
            try:
                panel.on_panel_activated()
            except Exception as e:
                print(f"Error in on_panel_activated: {e}")
