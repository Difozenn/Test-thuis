...
class AdminPanel(tk.Frame):
    ...
    def _build_excel_output_ui(self):
        ...
        # Get processing types and per-user output directories
        processing_types = settings.get('scanner_user_to_processing_type_map', {})
        ...
        for idx, user in enumerate(excel_users, start=1):
            ...
            if uses_user_path:
                ...
                # Per-user output directory - stored in user_output_dirs[username]
                current_path = user_output_dirs.get(user, '')
                self.excel_output_vars[user] = tk.StringVar(value=current_path)
                self.excel_entries[user] = ttk.Entry(settings_frame, textvariable=self.excel_output_vars[user], state='readonly')
                self.excel_entries[user].grid(row=idx, column=1, padx=5, pady=5, sticky='ew')
    ...
    def _browse_output_dir(self, username, path_var):
        """Browse for output directory for Excel files - saves to user_output_dirs"""
        directory = filedialog.askdirectory(title=f"Select Output Directory for {username}")
        if directory:
            path_var.set(directory)

            # Load current user_output_dirs and update
            settings = self._load_settings_from_api()
            user_output_dirs = settings.get('user_output_dirs', {})
            user_output_dirs[username] = directory

            # Save to database
            if self._save_settings_to_api({'user_output_dirs': user_output_dirs}):
                self.log_to_queue(f"Output directory voor {username} opgeslagen: {directory}")
            else:
                self.log_to_queue(f"FOUT: Kon output directory voor {username} niet opslaan")
    ...
    def _get_all_processing_types(self):
        """Get all available processing types from database settings"""
        settings = self._load_settings_from_api()

        # Load from database setting if available
        stored_types = settings.get('available_processing_types', [])
        if stored_types:
            return stored_types

        # Get types currently in use by configured users
        processing_map = settings.get('scanner_user_to_processing_type_map', {})
        types_in_use = set(processing_map.values())

        # Always include GEEN_PROCESSING
        types_in_use.add('GEEN_PROCESSING')

        return sorted(list(types_in_use))
            ...
            if user in self.excel_status_labels:
                ...
                if not directory:
                    self.excel_status_labels[user].config(
                        text=f"⚠️ {user}: Geen directory geconfigureerd",
                        foreground="orange"
                    )
                elif os.path.exists(directory) and os.path.isdir(directory):
                    label = "User Path" if uses_user_path else "Output directory"
                    self.excel_status_labels[user].config(
    ...
    def _create_user_config_tab(self, tab):
        ...
        self.available_processing_types = self._get_all_processing_types()
