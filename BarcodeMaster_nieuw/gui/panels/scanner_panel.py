...
class ScannerPanel(tk.Frame):
    ...
    def _start_new_batch_session_async(self):
        ...
        if self.session2_projects:
            self.background_project_label.config(text=self._format_projects_text(self.session2_projects))
        self.active_project_label.config(text="Project: --")
    ...
    def _format_projects_text(self, projects):
        """Format project list for display in session labels."""
        if not projects:
            return "Project: --"
        elif len(projects) == 1:
            return f"Project: {projects[0]}"
        else:
            return f"Projecten: {', '.join(projects)}"
    ...
    def load_config_values(self):
        ...
        self.scanner_panel_open_event_user_logic_active = config.get('scanner_panel_open_event_user_logic_active', {})
        self.scanner_user_to_processing_type_map = config.get('scanner_user_to_processing_type_map', {})
        self.available_processing_types = self._get_available_processing_types()
    ...
    def _get_available_processing_types(self):
        """Get available processing types from configured users"""
        settings = self._load_settings_from_api()
        processing_map = settings.get('scanner_user_to_processing_type_map', {})

        # Get unique processing types from configured users
        types_in_use = set(processing_map.values())

        # Always include GEEN_PROCESSING as default option
    ...
    def log_scan_event(self, code):
        ...
        try:
            ...
            if self.active_scan_session == 1 and self.session1_id:
                ...
                self.active_project_label.config(text=self._format_projects_text(self.session1_projects))
    ...
    def process_com_data(self, data):
        ...
        Routes to active panel if not scanner panel."""
        # Strip prefix before and including ';' (e.g. "4554656561;projectname" -> "projectname")
        if ';' in data:
            data = data.split(';', 1)[1]
        print(f"[ScannerPanel] Verwerken van COM data: {data}")

        # Check if another panel should receive this data
        if self.app and hasattr(self.app, 'active_panel') and self.app.active_panel != self:
            active_panel = self.app.active_panel
            # Route to archief panel if it's active and has receive_com_data method
            if hasattr(active_panel, 'receive_com_data'):
                print(f"[ScannerPanel] Routing COM data to {active_panel.__class__.__name__}")
                active_panel.receive_com_data(data)
                return

        # Process locally if scanner panel is active
        event_type = 'OPEN'  # Always use OPEN event type
