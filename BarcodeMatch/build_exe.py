...
'''
    ...
    def generate_version_json(self, exe_path):
        """Generate version.json for auto-updater deployment"""
        print("\nGenerating version.json for auto-updater...")
        ...
        # Get version from build_info.py
        version = "v1"
        try:
            build_info_path = self.root_dir / "build_info.py"
            if build_info_path.exists():
                with open(build_info_path, 'r') as f:
                    content = f.read()
                    import re
                    match = re.search(r'BUILD_NUMBER\s*=\s*["\']([^"\']+)["\']', content)
                    if match:
                        version = match.group(1)
        except Exception as e:
            print(f"Warning: Could not read version from build_info.py: {e}")
        ...
        version_data = {
            ...
            "sha256": file_hash,
            "changelog": f"- Build {version} ({datetime.now().strftime('%Y-%m-%d')})",
            "mandatory": False,
            "release_date": datetime.now().strftime('%Y-%m-%d')
        }

        version_json_path = self.dist_dir / "version.json"
        with open(version_json_path, 'w', encoding='utf-8') as f:
            json.dump(version_data, f, indent=4, ensure_ascii=False)

        print(f"version.json created: {version_json_path}")
...
"""
    ...
    def run(self):
        ...
        if self.build_exe():
            ...
            exe_path = self.dist_dir / exe_name
            if exe_path.exists():
                self.generate_version_json(exe_path)
            ...
            print("1. Copy the .exe and version.json to your update server")
            print("2. Update the changelog in version.json if needed")
            print("3. Users will see the update notification on next app start")
            print("=" * 50)
            return 0
        else:
            print("\n" + "=" * 50)
