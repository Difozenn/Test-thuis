...
class ScannerPanel(ttk.Frame):
    ...
    def _check_barcode(self, barcode):
        ...
        # Check for machine side indicator barcodes (@VL or @VR)
        if self._handle_machine_side_barcode(barcode):
            return  # Handled as side indicator, don't process further
    ...
    def _try_show_scan_highlight(self, scanned_barcode):
        ...
        try:
            ...
            # Build field list: skip Status column and edgeband columns
            fields = []
            dims = {}  # Track dimension values for 3D drawing
            edgebands = {}  # Track edgeband values for visual display on 3D box
            dim_keys = {'lengte', 'breedte', 'dikte', 'hoogte', 'length', 'width', 'height', 'thickness'}
            edgeband_map = {
                'afplak boven': 'boven', 'afplak onder': 'onder',
                'afplak links': 'links', 'afplak rechts': 'rechts',
            }
            for col, val in zip(tree_columns, tree_values):
                ...
                if col_lower in edgeband_map:
                    if str_val:
                        edgebands[edgeband_map[col_lower]] = str_val
                    continue
                fields.append((col, str_val))
                if col_lower in dim_keys and str_val:
                    try:
                        dims[col_lower] = float(str_val)
                    except ValueError:
                        pass
            ...
            if has_dims:
                ...
                def _redraw_box(event=None):
                    self._draw_3d_box(box_canvas, dims, bg_color, edgebands)
    ...
    # Edgeband style definitions - extensible for future edge types
    EDGEBAND_STYLES = {
    ...
    def _get_edgeband_style(self, label_text):
        """Match edgeband label to a style by keyword. Returns None if unrecognized."""
        text_lower = label_text.lower()
        for keyword, style in self.EDGEBAND_STYLES.items():
            if keyword in text_lower:
                return style
        return None

    def _draw_wavy_grain_on_face(self, canvas, corners, grain_color, seed=42):
        """Draw horizontal wavy grain lines on a quadrilateral face (tkinter canvas)."""
        tl, tr, br, bl = corners
        num_lines = max(int(abs(bl[1] - tl[1]) / 12), 2)
        num_lines = min(num_lines, 8)
        num_points = 15
        ...
        for i in range(1, num_lines + 1):
            ...
            if len(points) >= 4:
                canvas.create_line(*points, fill=grain_color, width=1, smooth=True)

    def _draw_3d_box(self, canvas, dims, bg_color, edgebands=None):
        ...
        # Margins for labels/arrows (extra space for callout labels when edgebands present)
        has_eb = bool(edgebands)
        margin_left = 90 if has_eb else 60
        margin_right = 100 if has_eb else 90
        margin_bottom = 80 if has_eb else 40
        margin_top = 55 if has_eb else 35
        ...
        # Resolve edgeband styles for visible faces
        onder_style = self._get_edgeband_style(edgebands['onder']) if 'onder' in edgebands else None
        rechts_style = self._get_edgeband_style(edgebands['rechts']) if 'rechts' in edgebands else None
        ...
        # Draw right side face (colored if rechts edgeband)
        if rechts_style:
            canvas.create_polygon(
                *f_br, *b_br, *b_tr, *f_tr,
                fill=rechts_style['fill_side'], outline='#555555', width=2
            )
            if rechts_style['grain']:
                self._draw_wavy_grain_on_face(canvas,
                    [f_tr, b_tr, b_br, f_br],
                    rechts_style.get('grain_color_side', rechts_style['grain_color']),
                    seed=99)
        ...
        # Draw front face (colored if onder edgeband)
        if onder_style:
            canvas.create_polygon(
                *f_bl, *f_br, *f_tr, *f_tl,
                fill=onder_style['fill'], outline='#555555', width=2
            )
            if onder_style['grain']:
                self._draw_wavy_grain_on_face(canvas,
                    [f_tl, f_tr, f_br, f_bl],
                    onder_style['grain_color'],
                    seed=42)
                # Redraw outline on top
                canvas.create_polygon(
                    *f_bl, *f_br, *f_tr, *f_tl,
        ...
        # Track sizing endpoint positions for edgeband callout spacing
        sizing_bottom_y = oy  # default: bottom of front face
        sizing_right_x = f_br[0]  # default: right edge of front face
        ...
        if length > 0:
            ...
            tid = canvas.create_text(mid_x, ay + 14, text=f"{length:g} mm",
                                     font=label_font, fill=label_color)
            bbox = canvas.bbox(tid)
            if bbox:
                sizing_bottom_y = bbox[3]  # bottom of lengte text
        ...
        if width > 0:
            ...
            tid = canvas.create_text(mid_x + 10, mid_y, text=f"{width:g} mm",
                                     font=label_font, fill=label_color)
            bbox = canvas.bbox(tid)
            if bbox:
                sizing_right_x = bbox[2]  # right edge of breedte text

        # Edgeband dotted leader-line callouts
        if edgebands:
            self._draw_edgeband_callouts(canvas, edgebands,
                                         f_tl, f_tr, f_bl, f_br,
                                         b_tl, b_tr, b_br, dl, dh,
                                         sizing_bottom_y, sizing_right_x)
    ...
    def _draw_edgeband_callouts(self, canvas, edgebands,
                                f_tl, f_tr, f_bl, f_br,
                                b_tl, b_tr, b_br, dl, dh,
                                sizing_bottom_y, sizing_right_x):
        """Draw dotted leader-line callouts for edgeband labels, spaced from sizing markers."""
        eb_font = ('Segoe UI', 9, 'bold')
        cc = '#555555'
        dot_r = 3
        # Gap between sizing markers and edgeband callout endpoints
        gap_pct = 0.06  # 6% of box length as gap from sizing
        scale_ref = max(dl, 50)
        gap = max(scale_ref * gap_pct, 8)
        diag = max(scale_ref * 0.06, 15)  # diagonal segment for boven
        horiz = max(scale_ref * 0.08, 20)  # horizontal leader length for links

        # BOVEN: dot on top edge midpoint, 45deg up-right, then horizontal dotted underline + text
        if 'boven' in edgebands:
            mx = (b_tl[0] + b_tr[0]) / 2
            my = (b_tl[1] + b_tr[1]) / 2
            canvas.create_oval(mx - dot_r, my - dot_r, mx + dot_r, my + dot_r, fill=cc, outline=cc)
            p2x = mx + diag * 0.707
            p2y = my - diag * 0.707
            canvas.create_line(mx, my, p2x, p2y, fill=cc, width=1, dash=(3, 3))
            txt = edgebands['boven']
            tid = canvas.create_text(p2x + 4, p2y - 2, text=txt, font=eb_font,
                                     fill='#333333', anchor='sw')
            bbox = canvas.bbox(tid)
            if bbox:
                canvas.create_line(p2x, p2y, bbox[2] + 4, p2y, fill=cc, width=1, dash=(3, 3))

        # LINKS: dot on left top edge midpoint, straight left, dotted underline + text
        if 'links' in edgebands:
            ...
            txt = edgebands['links']
            tid = canvas.create_text(p2x - 4, p2y - 2, text=txt, font=eb_font,
                                     fill='#333333', anchor='se')
            bbox = canvas.bbox(tid)
            if bbox:
                canvas.create_line(bbox[0] - 4, p2y, p2x, p2y, fill=cc, width=1, dash=(3, 3))

        # RECHTS: dot at center of right side plane, endpoint at gap% past sizing right edge
        if 'rechts' in edgebands:
            sx = (f_tr[0] + f_br[0] + b_tr[0] + b_br[0]) / 4
            sy = (f_tr[1] + f_br[1] + b_tr[1] + b_br[1]) / 4
            canvas.create_oval(sx - dot_r, sy - dot_r, sx + dot_r, sy + dot_r, fill=cc, outline=cc)
            p2x = sizing_right_x + gap
            p2y = sy
            canvas.create_line(sx, sy, p2x, p2y, fill=cc, width=1, dash=(3, 3))
        ...
        # ONDER: dot at center of front face, 45deg down-right, endpoint at gap% below sizing bottom
        if 'onder' in edgebands:
            sx = (f_bl[0] + f_br[0]) / 2
            sy = (f_tl[1] + f_bl[1]) / 2
            canvas.create_oval(sx - dot_r, sy - dot_r, sx + dot_r, sy + dot_r, fill=cc, outline=cc)
            # Target Y: sizing bottom + gap
            target_y = sizing_bottom_y + gap
            # Calculate diagonal length needed to reach that Y from startpoint
            dy_needed = target_y - sy
            diag_onder = max(dy_needed / 0.707, 40)
            p2x = sx + diag_onder * 0.707
            p2y = sy + diag_onder * 0.707
            canvas.create_line(sx, sy, p2x, p2y, fill=cc, width=1, dash=(3, 3))
            txt = edgebands['onder']
            tid = canvas.create_text(p2x + 4, p2y - 2, text=txt, font=eb_font,
                                     fill='#333333', anchor='sw')
    ...
    def _show_image_in_viewer(self, image_path, image_filename, scanned_item=None):
        """Shows an image in the custom image viewer window."""
        try:
            # Clear machine side indicator when opening a new image
            self.current_machine_side = None
            print(f"[SIDE_INDICATOR] _show_image_in_viewer: cleared machine side, opening new image")
    ...
    def _update_image_display(self):
        ...
        try:
            ...
            # Draw machine side indicator (L shape) if @VL or @VR is in scanned item
            self._draw_machine_side_indicator(new_width, new_height)

        except Exception as e:
            self._log(f"[WARN] Fout bij weergeven afbeelding: {e}")

    def _handle_machine_side_barcode(self, barcode):
        """Handles @VL and @VR barcodes to show machine side indicator on open image viewer.

        Returns True if the barcode was handled as a side indicator, False otherwise.
        """
        barcode_upper = barcode.strip().upper()
        print(f"[SIDE_INDICATOR] _handle_machine_side_barcode called with: '{barcode}' -> '{barcode_upper}'")

        # Check if it's a side indicator barcode
        if barcode_upper not in ('@VL', '@VR'):
            print(f"[SIDE_INDICATOR] Not a side indicator barcode")
            return False

        # Check if image viewer is open
        if self.image_viewer_window is None or not self.image_viewer_window.winfo_exists():
            print(f"[SIDE_INDICATOR] Image viewer not open, ignoring side indicator")
            self._log(f"[INFO] {barcode_upper} gescand maar geen afbeelding open")
            return True  # Still return True to prevent "not found" error

        # Store the side indicator
        self.current_machine_side = 'left' if barcode_upper == '@VL' else 'right'
        print(f"[SIDE_INDICATOR] Setting machine side to: {self.current_machine_side}")

        # Redraw the image with the side indicator
        self._update_image_display()

        self._log(f"✓ Machine zijde: {'LINKS' if self.current_machine_side == 'left' else 'RECHTS'}")
        return True

    def _draw_machine_side_indicator(self, img_width, img_height):
        """Draws an L-shaped machine side indicator on the image viewer canvas.

        @VL = Left side: L shape on left bottom (|_)
        @VR = Right side: Mirrored L shape on right bottom (_|)
        """
        machine_side = getattr(self, 'current_machine_side', None)
        print(f"[SIDE_INDICATOR] _draw_machine_side_indicator called with img_width={img_width}, img_height={img_height}")
        print(f"[SIDE_INDICATOR] current_machine_side = '{machine_side}'")

        if not machine_side:
            print("[SIDE_INDICATOR] No machine_side set, returning")
            return

        is_left = machine_side == 'left'
        is_right = machine_side == 'right'
        print(f"[SIDE_INDICATOR] is_left={is_left}, is_right={is_right}")

        print(f"[SIDE_INDICATOR] Drawing indicator for {'LEFT' if is_left else 'RIGHT'} side")

        # L shape dimensions (scaled with image but with min/max limits)
        base_size = min(img_width, img_height) * 0.15  # 15% of smallest dimension
        l_height = max(80, min(200, base_size))  # Vertical line height
        l_width = max(60, min(150, base_size * 0.75))  # Horizontal line width
        line_thickness = max(8, min(20, base_size * 0.1))  # Line thickness
        margin = 20  # Distance from edge

        # Colors
        fill_color = '#FF0000'  # Red color for visibility
        outline_color = '#8B0000'  # Dark red outline

        if is_left:
            # Left side: L shape |_ on bottom-left
            # Vertical line
            x1 = margin
            y1 = img_height - margin - l_height
            x2 = margin + line_thickness
            y2 = img_height - margin
            self.image_canvas.create_rectangle(x1, y1, x2, y2, fill=fill_color, outline=outline_color, width=2, tags="side_indicator")

            # Horizontal line
            x1 = margin
            y1 = img_height - margin - line_thickness
            x2 = margin + l_width
            y2 = img_height - margin
            self.image_canvas.create_rectangle(x1, y1, x2, y2, fill=fill_color, outline=outline_color, width=2, tags="side_indicator")

            # Add "L" text label
            self.image_canvas.create_text(margin + l_width + 15, img_height - margin - l_height/2,
                                          text="L", font=('Arial', 24, 'bold'), fill=fill_color, tags="side_indicator")

        elif is_right:
            # Right side: Mirrored L shape _| on bottom-right
            # Vertical line
            x1 = img_width - margin - line_thickness
            y1 = img_height - margin - l_height
            x2 = img_width - margin
            y2 = img_height - margin
            self.image_canvas.create_rectangle(x1, y1, x2, y2, fill=fill_color, outline=outline_color, width=2, tags="side_indicator")
            ...
            # Add "R" text label
            self.image_canvas.create_text(img_width - margin - l_width - 15, img_height - margin - l_height/2,
                                          text="R", font=('Arial', 24, 'bold'), fill=fill_color, tags="side_indicator")
    ...
    def _find_barcode_in_pdf(self, pdf_path, barcode):
        ...
        try:
            ...
            # Remove specific suffixes to find match in PDF
            # e.g., "MW_01DR_L_01_L.HOP" -> "MW_01DR_L_01_L" (after extension removal) -> "MW_01DR_L_01" (after suffix removal)
            # Check longer suffixes first to avoid partial matches
            suffixes_to_remove = ['XLV', 'X_L', 'LV', '_L']
            for suffix in suffixes_to_remove:
                if clean_barcode.endswith(suffix):
                    clean_barcode = clean_barcode[:-len(suffix)]
                    print(f"[STUKTEKENING] Removed suffix '{suffix}' from barcode")
                    break
