...
SEEN_ENTRIES_LOCK_FILE = f"{SEEN_ENTRIES_FILE}.lock"
...
class ManualAddApp(tk.Tk):
    ...
    def submit(self):
        ...
        if not cat or cat not in category_list:
            ...
            return
        # Add events to .seen_entries.json with file locking
        lock = FileLock(SEEN_ENTRIES_LOCK_FILE)
        try:
            ...
            messagebox.showerror('Fout', 'Kon geen toegang krijgen tot het logbestand. Probeer het opnieuw.')
