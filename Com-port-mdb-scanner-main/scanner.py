...
class FileScannerFrame(ttk.Frame):
    def __init__(self, parent):
        ...
        scan_mode_combo_tooltip = Tooltip(scan_mode_combo, "Kies scanmodus: OPUS (.hop/.hops) of GANNOMAT (.mdb)") # Already Dutch
    ...
    def save_config(self):
        ...
        if base_dir:
            try:
                ...
                if hasattr(self, 'scan_mode_var'):
                    config['default_scan_mode'] = self.scan_mode_var.get()
                if hasattr(self, 'scanner_type_var'):
                    config['default_scanner_type'] = self.scanner_type_var.get()
...
class COMPortComparator:
    def __init__(self, root):
        ...
        # Add Importeren tab with icon only
        self.importeren_tab = ttk.Frame(self.notebook)
        try:
            from tkinter import PhotoImage
            importeren_icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "importeren.png")
            try:
                from PIL import Image, ImageTk
                pil_img = Image.open(importeren_icon_path).resize((75, 75), Image.LANCZOS)
                self.importeren_icon = ImageTk.PhotoImage(pil_img)
            except ImportError:
                tmp_img = PhotoImage(file=importeren_icon_path)
                self.importeren_icon = tmp_img.subsample(max(1, tmp_img.width() // 75), max(1, tmp_img.height() // 75))
            self.notebook.add(self.importeren_tab, text="", image=self.importeren_icon)
            self.notebook.tab(0, state='normal')  # Ensure tab is enabled
        except Exception as e:
            self.importeren_icon = None
        ...
        # Load scanner icon, scale to 75x75 using Pillow if available
        try:
            from tkinter import PhotoImage
            scanner_icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "scanner.png")
            try:
                from PIL import Image, ImageTk
                pil_img = Image.open(scanner_icon_path).resize((75, 75), Image.LANCZOS)
                self.scanner_icon = ImageTk.PhotoImage(pil_img)
            except ImportError:
                # Pillow not available, fallback to Tkinter scaling (may look worse)
                tmp_img = PhotoImage(file=scanner_icon_path)
                self.scanner_icon = tmp_img.subsample(max(1, tmp_img.width() // 75), max(1, tmp_img.height() // 75))
            self.notebook.add(self.scanner_tab, text="", image=self.scanner_icon)
            self.notebook.tab(1, state='normal')
        except Exception as e:
            self.scanner_icon = None
            self.notebook.add(self.scanner_tab, text="")

        # --- Email Tab ---
        self.email_tab = ttk.Frame(self.notebook)
        try:
            from tkinter import PhotoImage
            email_icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "email.png")
            try:
                from PIL import Image, ImageTk
                pil_img = Image.open(email_icon_path).resize((75, 75), Image.LANCZOS)
                self.email_icon = ImageTk.PhotoImage(pil_img)
            except ImportError:
                tmp_img = PhotoImage(file=email_icon_path)
                self.email_icon = tmp_img.subsample(max(1, tmp_img.width() // 75), max(1, tmp_img.height() // 75))
            self.notebook.add(self.email_tab, text=" ", image=self.email_icon)
            self.notebook.tab(2, state='normal')
        except Exception as e:
            self.email_icon = None
            self.notebook.add(self.email_tab, text="Email")

        # Build Scanner Tab Widgets
        ...
        # --- Make Treeview and log area vertically resizable ---
        self.results_paned = ttk.PanedWindow(self.results_frame, orient=tk.VERTICAL)
        self.results_paned.grid(row=0, column=0, sticky=(tk.N, tk.S, tk.E, tk.W))

        # Treeview for scanned items
        self.scanned_frame = ttk.Frame(self.results_paned)
        ...
        # Remove the old results_notebook code
    ...
    def send_log_via_email(self, subject="OPUS Scan Log"):
        """Send the scan_log.txt contents as an email body using config.json settings."""
        import traceback
        try:
            email_cfg = load_email_config()
    ...
    def _compare_thread(self):
        ...
        try:
            ...
            if total_items > scanned_items:
                ...
                filename_no_ext = os.path.splitext(excel_file)[0]
                with open("scan_log.txt", "a", encoding="utf-8") as logf:
                    logf.write(f"[{time.strftime('%Y-%m-%d %H:%M')}] '{filename_no_ext}' Is afgemeld aan OPUS\n")
                # Send log via email
                self.send_log_via_email()
        ...
        try:
            ...
            if hasattr(app, 'save_scanner_type_to_config'):
                ...
                if os.path.exists(config_file):
                    with open(config_file, 'r') as f:
                        config = json.load(f)
                config['default_scanner_type'] = app.scanner_type_var.get()
                with open(config_file, 'w') as f:
                    json.dump(config, f, indent=4)
            # Save email settings
