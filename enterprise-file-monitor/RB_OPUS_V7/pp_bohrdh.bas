...
' -- File     \RB_OH_OPUS_V7\pp_bohr.bas
' -- 
' -----------------------------------------
' -- 
...
Dim wp As TWPI
	...
	If Not Marker.FirstTime_DH_Drilling Then
		...
		WCNC_Write_DHCode(tools,False)
		' ----------------------------------------------------
	End If
	
	' -- etwaige Drehzahlaenderung
	DH_Speed(Driller)
...
Dim wp As TWPI
Dim Unterk_abs, Tiefe_abs As Double ' Oberk_abs,
Dim ref_IPZ As Double 
	...
	'Oberk_abs = Unterk_abs + wp.WPz  ' Oberkante Werkstueck
	' 
	ref_IPZ = DH_View0.IPZ + actt.dh.CenterZ   ' MW 27.02.2020 !!!!
	Tiefe_abs = ref_IPZ + Depth
	...
	GetParams_Drills_Edge(MUF,FF,SIC_G0,DFI,DFS,DWELL_TIME,dh,Driller)


	wcnccom("Drill Cycle 20")
	wcnccom("Unterkante WKS:"+ftos(Unterk_abs)+ "  WP:"+wp.SName+" WPDicke:"+ftos(wp.wpz))
	wcnccom("Additives ZMass:"+ftos(GetAddZSic))
    ...
    End If
   	If Depth>=zmax Then 
	    ...
	    If breakthrough Then
	        ...
	        wcnc(G1+Move(PPVX,PPVY,-(ref_IPZ-Unterk_abs)+DFS,Driller.v,0)) 
	...
	Else
	    ...
	    If breakthrough Then
		    ' --- Bohrung geht durch ---
		    Count=(-(ref_IPZ-Unterk_abs)+DFS)\zmax
			For I = 1 To Count Step 1
				ActDepth=I*zmax
			    wcnc(G1+Move(PPVX,PPVY,ActDepth,Driller.v,0))
		        If (Marker.G0_Up_DH) Then
...
Dim SIC_G0 As Double   ' distance above hole 
Dim FF As Double       ' fast feedrate position to Sic above hole
Dim MUF As Double      ' move up feedrate 
Dim DWELL_TIME As Double  'Verweilzeit bei Topfband       
Dim wp As TWPI
    ...
    WCNC_SUB("G04",DWELL_TIME)        ' Verweilzeit 
...
' --
' -- Ermittlung der Parameter fuer Bohrkopfbohren
' -- DFI          ' ==>  Eintauchvorschub
' -- DFS          ' ==>  Austrittdistanz durchbohren
' -- SIC_G0       ' ==>  SicG0 - Werkstueck Distanz
' -- FF           ' ==>  fast feedrate position to Sic above hole
' -- MUF          ' ==>  Austauchvorschub - durchbohren
' -- DWELL_TIME   ' ==>  Verweilzeit Topfbandbohren auf Bohrgrund
' --  
...
Const ID_MUF = 1004    ' Rueckzugsvorschub 
Const ID_DWELL_TIME = 1005    ' Verweilzeit

Const SEC_VAL1 = 20    ' Grenzmass Wert (anbohren,durchbohren)
Const SEC_VAL2 = 0.01  ' Verweilzeit Topfbohren
	...
	If Not actt.t_DH.DrillingHead.Additions.GetAddition_ID(ID_DFI_Edge) Is Nothing Then
		...
		If StrToFloat(tmp)>=0 Then
			...
			DFI = -Abs(StrToFloat(tmp))
			If (DFI < (-SEC_VAL1)) Then
				' SecCheck
				DFI = -SEC_VAL1
			End If			 
		End If
	...
	If Not actt.t_DH.DrillingHead.Additions.GetAddition_ID(ID_DWELL_TIME) Is Nothing Then
		...
		If StrToFloat(tmp)>0 Then
			DWELL_TIME = Abs(StrToFloat(tmp))
			If (DWELL_TIME < SEC_VAL2) Then
				DWELL_TIME = SEC_VAL2
			End If			 
		End If
	End If
...
Function DH_Speed(Driller As tDriller)


		' --
		' MW 12.02.2020 keine CH_SPINDEL CYCLE FUER BOHRKOPF
'		If (Not equal(Marker.LastSpeed,Driller.Speed))  And (Driller.Speed <> 0) Then
