...
'	If DH_View0.IPZ <> FinishedPart.Z Then
		...
		Sic_Z = Sic_Z + (diff_eb) + GetAddZSic
...
'	End If
    ...
    If (DFI>Depth) And Not (equal(Driller.ve,Driller.v)) Then
       ...
       Move(PPVX,PPVY,DFI,Driller.ve,0)
    End If
   	If Depth>=zmax Then 
	    'Drilling all depth
	    Move(PPVX,PPVY,Depth,Driller.v,0)
	Else
		' Bohren mit maximaler Zustellung
		Count=Depth\zmax
		For I = 1 To Count Step 1
			ActDepth=I*zmax
		    Move(PPVX,PPVY,ActDepth,Driller.v,0)
		    ' jetzt zurück zum ausräumen
			Move(PPVX,PPVY,0,FF,0)
		Next I
    ...
    End If
   	If Depth>=zmax Then 
	    ...
	    If breakthrough Then
	        ...
	        Move(PPVX,PPVY,-(DH_View0.IPZ-Unterk_abs)+DFS,Driller.v,0)
	    	'jetzt durchbohren mit Austauchvorschub
		    Move(PPVX,PPVY,Depth,Driller.va,0)
		Else
		    Move(PPVX,PPVY,Depth,Driller.v,0)
		End If
...
'	wcnccom("Drill Cycle horizontal")
    ...
    Move(PPVX,PPVY,Actt.t_dh.SecurityHorz,0,0)
	...
	'Go to safety position on the view
    Move(PPVX,PPVY,PPVZ+SIC,FF,0)


    
    If (DFI>Depth) And ((DFI>zmax) Or (DFlag=0)) And Not equal(Driller.v,Driller.ve) Then
       'Drilling with the surface feed
       Move(PPVX,PPVY,PPVZ+DFI,Driller.ve,0)
    End If
    
    If DFlag=0 Then
      'Drilling all depth
      Move(PPVX,PPVY,PPVZ+Depth,Driller.v,0)
    Else  
       'Drilling with a maximum depth (ZMax)
      Count=Depth\zmax  
      For I = 1 To Count Step 1
        ActDepth=I*zmax
        Move(PPVX,PPVY,PPVZ+ActDepth,Driller.v,0)
        
        ' Rückzug auf 0
        Move(PPVX,PPVY,PPVZ,FF,0)
