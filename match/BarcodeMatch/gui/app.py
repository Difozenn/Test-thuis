...
class BarcodeMatchApp:
    def __init__(self, root, skip_preload=False):
        ...
        self.root.after(2000, self._start_update_checker)
    ...
    def _show_update_notification(self, version, changelog):
        ...
        if hasattr(self.root, 'show_update_notification'):
            self.root.show_update_notification(version, changelog)

    def test_update_notification(self, version="v99"):
        """Manual test function to trigger update notification"""
        print(f'[UPDATE TEST] Triggering notification for: {version}')
        if hasattr(self.root, 'show_update_notification'):
            self.root.show_update_notification(version, "Test changelog")
        else:
            print('[UPDATE TEST] Notification system not initialized')
